...
module Machine : MACHINE = struct
  (* type ukazi = (kljuc, naslednje_stanje, znak , smer) *)
  type ukazi = char * state * char * direction
  type t = state * (state * (ukazi list) list)

  let make state state_list = 
    let rec make' state state_list acc =
      match state_list with
      | [] -> (state, acc)
      | x :: xs -> make' state xs ((x,[]) :: acc)
    in
    make' state state_list []
